...
$live_video = $post_title = $post_description = $video_uploadDate = $video_thumbnailUrl = $video_contentUrl = $video_duration = $video_name = $video_startDate = $video_endDate = $post_content = $post_document_replace = "";
$live_video_err = $post_title_err  = $post_description_err  = $video_uploadDate_err = $video_thumbnailUrl_err = $video_contentUrl_err = $video_duration_err = $video_name_err = $video_startDate_err = $video_endDate_err = $post_content_err  = $post_document_replace_err  = "";
 
// Processing form data when form is submitted
if($_SERVER["REQUEST_METHOD"] == "POST"){
    // Validate name
// Validate live_video
    $input_live_video = trim($_POST["live_video"]);
    if(empty($input_live_video)){
        $live_video_err = "Please enter an live_video.";     
    } else{
        $live_video = $input_live_video;
    }   
    // Validate post_title
...
// Validate post_document_replace
    ...
    if(empty($live_video_err) && empty($post_title_err) && empty($post_description_err) && empty($video_uploadDate_err) && empty($video_thumbnailUrl_err) && empty($video_contentUrl_err) && empty($video_duration_err) && empty($video_name_err) && empty($video_startDate_err) && empty($video_endDate_err) && empty($post_content_err) && empty($post_document_replace_err)){
        // Prepare an insert statement
        $sql = "INSERT INTO live_posts (live_video, post_title, post_description, video_uploadDate, video_thumbnailUrl, video_contentUrl, video_duration, video_name, video_startDate, video_endDate, post_content, post_document_replace) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssssss", $param_live_video, $param_post_title, $param_post_description, $param_video_uploadDate, $param_video_thumbnailUrl, $param_video_contentUrl, $param_video_duration, $param_video_name, $param_video_startDate, $param_video_endDate, $param_post_content, $param_post_document_replace);
            
            // Set parameters
            $param_live_video = $live_video;
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="form-group">
                            ...
                            <textarea name="live_video" class="form-control <?php echo (!empty($live_video_err)) ? 'is-invalid' : ''; ?>"><?php echo $live_video; ?></textarea>
                            <span class="invalid-feedback"><?php echo $live_video_err;?></span>
                        </div>
                        <div class="form-group">
